    ...
    siv.add_layer(
        Dialog::around(
            LinearLayout::vertical()
                ...
                .child(TextView::new(""))
                .child(TextView::new("Generate Solana wallet addresses with custom prefixes or suffixes").h_align(HAlign::Center))
                .child(TextView::new(""))
    ...
    siv.add_layer(
        ...
        .button("Generate", move |s| {
            ...
            if pattern.len() < 3 || pattern.len() > 8 {
                s.add_layer(
                    Dialog::info("Pattern must be between 3 and 8 characters long.")
                );
                return;
            }
            
            // Launch the generation process
            generate_address(s, server.to_string(), pattern.to_string(), position.to_string());
        })
        .button("Cancel", |s| {
            s.pop_layer();
        }),
...
async fn start_job(server: String, pattern: String, position: String) -> Result<String, reqwest::Error> {
...
async fn check_job_status(server: String, job_id: String) -> Result<(String, Option<(String, String)>), reqwest::Error> {
    let client = reqwest::Client::new();
    let res = client
...
fn generate_address(siv: &mut Cursive, server: String, pattern: String, position: String) {
    ...
    siv.add_layer(
        Dialog::around(
            LinearLayout::vertical()
                .child(TextView::new(format!("Generating address with {} '{}'...", position, pattern)))
                .child(TextView::new(""))
    ...
    thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            match start_job(server_clone.clone(), pattern_clone, position_clone).await {
                Ok(job_id) => {
                    if job_id.is_empty() {
                        sender.send(("error".to_string(), "Failed to start job".to_string(), None)).unwrap();
                        return;
                    }
                            ...
                            Ok((status, result)) => {
                                if status == "complete" {
                                    if let Some((pub_key, priv_key)) = result {
                                        ...
                                        sender.send(("complete".to_string(), 
                                                    format!("Found in {:.2}s", elapsed),
                                                    Some((pub_key, priv_key)))).unwrap();
                                        break;
                                    }
                                } else if status == "error" {
                                    sender.send(("error".to_string(), "Error checking job status".to_string(), None)).unwrap();
                                    break;
                                } else {
                                    // Still running - update with time elapsed
                                    let elapsed = start_time.elapsed().as_secs_f32();
                                    sender.send(("running".to_string(), 
                                                format!("Running... {:.2}s elapsed", elapsed),
                                                None)).unwrap();
                                }
                            },
                            Err(e) => {
                                sender.send(("error".to_string(), format!("Error: {}", e), None)).unwrap();
                                break;
                            }
                        }
            ...
            if status == "complete" {
                if let Some((pub_key, priv_key)) = keys {
                    ...
                    s.add_layer(
                        Dialog::around(
                            LinearLayout::vertical()
                                .child(TextView::new("✅ Address found!").style(cursive::theme::Effect::Bold))
                                .child(TextView::new(message))
                                .child(TextView::new(""))
                                .child(TextView::new("📝 PUBLIC KEY:").style(cursive::theme::Effect::Bold))
                                .child(TextView::new(pub_key.clone()))
                                .child(TextView::new(""))
                                .child(TextView::new("🔑 PRIVATE KEY:").style(cursive::theme::Effect::Bold))
                                .child(TextView::new(priv_key.clone()))
                                .child(TextView::new(""))
                                .child(TextView::new("⚠️ IMPORTANT: Save your private key securely!").style(cursive::theme::Effect::Bold))
                        )
